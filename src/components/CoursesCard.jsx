export const CoursesCard = ({coursesCard}) => {
    ...
    return (
        <div >
            <div className={'bg-[#F1F1F3] md:p-[50px] '}>
                <img className={'hover:scale-110 transition-all duration-300 ease-in-out'} src={img} alt=""/>

                <div className={'md:flex justify-between mx-[10px] items-center mt-[30px]'}>
                    <div className={'md:flex gap-4  '}>
                        {tags.map(tag => <div className={'flex flex-row'}><h1
                            className={'bg-white px-[16px] mt-[10px] py-[10px]'}>{tag}</h1></div>)}
                    </div>
                    <p>By John Smith</p>
                </div>
                ...
                <button className={'bg-[#002868] font-medium w-[300px] md:w-full py-[18px] text-white mb-[50px] leading-[150%] hover:bg-blue-800'}>Get it Now</button>
            </div>
